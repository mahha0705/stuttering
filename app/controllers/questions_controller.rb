...
class QuestionsController < ApplicationController
  ...
  def index
    @questions = if params[:sort].present?
                   Question.select_tag_questions(params[:sort], params[:page]).newest
                 else
                   Question.newest.page(params[:page])
                 end
  end

  def search
    # タグを選択したか判定
    @questions = if params[:seacrh_sort].present?
                   Question.select_tag_questions(params[:seacrh_sort], params[:page])
                 else
                   Question.page(params[:page])
                 end
    # 回答の有無判定
